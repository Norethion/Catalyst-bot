...
exports.run = function(bot, message, params, perms) {
  ...
  if (params.length < 0) {
    message.channel.send(
      `Doğru kullanım : \n ${prefix}zamanlayıcı saat:dakika:saniye │ ${prefix}zamanlayıcı 00:10:00 gibi`
    );
    return;
  }
  try {
    ...
    message.channel.send(
      `Lütfen zamanlayıcıyı hangi zamana ayarliyacağını belirt. \`00:10:00\``
    );
    return;
  }
  var zaman = params[0].split(":");
  if (isNaN(parseInt(zaman[0]))) {
    message.channel.send(
      "Doğru kullanım : \n *zamanlayıcı saat:dakika:saniye │ *zamanlayıcı 00:15:00 gibi..."
    );
    return;
  }
  if (
    isNaN(parseInt(zaman[1])) ||
    !(parseInt(zaman[1]) < 60) ||
    !(parseInt(zaman[1]) >= 0)
  ) {
    message.channel.send(
  ...
  var schTime =
    Date.now() +
    parseInt(zaman[0]) * 3600000 +
    parseInt(zaman[1]) * 60000 +
    parseInt(zaman[2]) * 1000;
    ...
    .then(msg => {
      var interval = setInterval(() => {
        ...
        if (schTime - Date.now() + 1 < 0) {
          ...
          if (min === 60) min = 0;
          let hour = Math.floor(
            (Math.floor((schTime - Date.now()) / 1000) + 1) / 3600
          );
