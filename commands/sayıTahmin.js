...
exports.run = async (client, message, args) => {
  ...
  if (this.games.has(message.channel.id))
    await message.reply("Kanal başına sadece bir düello meydana gelebilir.");
  ...
  while (!uwu && haklar !== 0) {
    ...
    if (!response.first()) {
      ...
      message.channel.send(
        `${message.author}, :shrug: Kaybettin! Sayı: \`${fixedlisonuç}\` :shrug: `
      );
    }
  ...
  if (uwu) {
    this.games.delete(message.channel.id);
    await message.channel.send(
      `${message.author}, :tada:  Doğru Tahmin! Sayı: \`${fixedlisonuç}\` :tada:`
    );
    try {
    } catch (e) {
      this.games.delete(message.channel.id);
