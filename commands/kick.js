...
module.exports.run = async (bot, message, args) => {
  ...
  // No args
  if (!args[0]) {
    return message.reply("Lütfen bir kişi belirtin.").then(m => m.delete(5000));
  }

  // No reason
  if (!args[1]) {
    return message
      .reply("Lütfen bir sebep belirtin.")
  ...
  if (!message.member.hasPermission("KICK_MEMBERS")) {
    return message
      .reply("❌ Üyeleri atmak için izniniz yok.")
      .then(m => m.delete(5000));
  }
  ...
  if (!message.guild.me.hasPermission("KICK_MEMBERS")) {
    return message
      .reply("❌ Üyeleri atmak için iznim yok.")
      .then(m => m.delete(5000));
  }
  ...
  if (toKick.id === message.author.id) {
    return message.reply("Kendini atmazsın...").then(m => m.delete(5000));
  }

  // Check if the user's kickable
  if (!toKick.kickable) {
    return message
      .reply("Sanırım rol hiyerarşisi nedeniyle o kişiyi atamam.")
      .then(m => m.delete(5000));
  }
  ...
  await message.channel.send(promptEmbed).then(async msg => {
    ...
    if (emoji === "✅") {
      ...
      toKick.kick(args.slice(1).join(" ")).catch(err => {
        if (err)
          return message.channel.send(
            `Peki .... atma işe yaramadı. İşte hata ${err}`
          );
      });
      ...
      message.reply(`Atma iptal edildi.`).then(m => m.delete(10000));
    }
  });
};
